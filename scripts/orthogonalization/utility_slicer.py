...
class SegmentEditorManager:
    ...
    @log_method_call
    def copy_segment(self, new_segment_name, from_segment_name_or_id):
        # 1. Crea nuovo segmento vuoto
        ...
        # 2. Risolvi l'ID del segmento sorgente (Gestione robusta nomi)
        seg_logic = self.segmentation_node.GetSegmentation()
        from_id = from_segment_name_or_id
        
        # Se l'ID non esiste, cercalo per nome
        if not seg_logic.GetSegment(from_id):
            found_id = seg_logic.GetSegmentIdBySegmentName(from_segment_name_or_id)
            if found_id:
                from_id = found_id
            else:
                # Fallback: Cerca parziale (es. "aorta" in "Segment_1_Aorta")
                logging.warning(f"Segment '{from_segment_name_or_id}' not found by exact name. Fuzzy search...")
                for i in range(seg_logic.GetNumberOfSegments()):
                    sid = seg_logic.GetNthSegmentID(i)
                    if from_segment_name_or_id.lower() in seg_logic.GetSegment(sid).GetName().lower():
                        from_id = sid
                        logging.info(f"Fuzzy match found: Use {sid}")
                        break
        
        if not seg_logic.GetSegment(from_id):
             logging.error(f"CRITICAL: Source segment '{from_segment_name_or_id}' NOT FOUND anywhere.")
             return None
    ...
    def subtract_segment(self, segment_id, subtract_id):
        # Risolvi ID target
        seg_ptr = self.segmentation_node.GetSegmentation()
        target_id = segment_id
        if not seg_ptr.GetSegment(target_id):
            target_id = seg_ptr.GetSegmentIdBySegmentName(segment_id)
        self.set_segment(target_id)
        
        # Risolvi ID da sottrarre
        mod_id = subtract_id
        if not seg_ptr.GetSegment(mod_id):
             mod_id = seg_ptr.GetSegmentIdBySegmentName(subtract_id)

        self.apply_effect("Logical operators", {
            "Operation": "SUBTRACT",
            "ModifierSegmentID": mod_id,
            "BypassMasking": 1
        })

    @log_method_call
    def add_segment(self, segment_id, add_name_or_id):
        # Risolvi ID target
        seg_ptr = self.segmentation_node.GetSegmentation()
        target_id = segment_id
        if not seg_ptr.GetSegment(target_id):
        ...
        # Risolvi ID da aggiungere (con ricerca fuzzy)
        mod_id = add_name_or_id
        if not seg_ptr.GetSegment(mod_id):
            found = seg_ptr.GetSegmentIdBySegmentName(add_name_or_id)
            if found:
                mod_id = found
            else:
                for i in range(seg_ptr.GetNumberOfSegments()):
                    sid = seg_ptr.GetNthSegmentID(i)
                    if add_name_or_id.lower() in seg_ptr.GetSegment(sid).GetName().lower():
                        mod_id = sid
                        break
        
        if not seg_ptr.GetSegment(mod_id):
            logging.error(f"Cannot add segment: '{add_name_or_id}' not found.")
            return

        # USA "UNION" INVECE DI "ADD"
        self.apply_effect("Logical operators", {
            "Operation": "UNION", # <--- CORREZIONE QUI
            "ModifierSegmentID": mod_id,
            "BypassMasking": 1
        })
    @log_method_call
...
def load_data(folder_path, case_id, setting, segmentations):
    ...
    # 1. Carica il volume CT e ottieni il nodo
    ct_node = slicer.util.loadVolume(ct_path)

    # 2. Crea un nodo "contenitore" unico per tutte le segmentazioni
    combined_seg_node = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLSegmentationNode", "Segmentation")
    combined_seg_node.CreateDefaultDisplayNodes()
    if ct_node:
        combined_seg_node.SetReferenceImageGeometryParameterFromVolumeNode(ct_node)
    ...
    for seg_name in segmentations:
        ...
        # Carica in un nodo temporaneo
        temp_node = slicer.util.loadSegmentation(seg_path)
        if not temp_node:
            logging.warning(f"Failed to load {seg_filename}")
            continue

        # Sposta il segmento nel nodo contenitore
        src_seg = temp_node.GetSegmentation()
        dst_seg = combined_seg_node.GetSegmentation()
        
        # Copia segmenti e rinominali
        for i in range(src_seg.GetNumberOfSegments()):
            src_id = src_seg.GetNthSegmentID(i)
            dst_seg.CopySegmentFromSegmentation(src_seg, src_id)
            
            # L'ultimo segmento aggiunto è quello nuovo
            new_id = dst_seg.GetNthSegmentID(dst_seg.GetNumberOfSegments() - 1)
            # Rinomina FORZATAMENTE con il nome che le tue funzioni si aspettano (es. "aorta")
            dst_seg.GetSegment(new_id).SetName(seg_name)
            logging.info(f"Loaded and renamed segment: {seg_name}")

        # Rimuovi il nodo temporaneo
        slicer.mrmlScene.RemoveNode(temp_node)
    
    # Restituisci i nodi per poterli usare nello script principale
    return ct_node, combined_seg_node
